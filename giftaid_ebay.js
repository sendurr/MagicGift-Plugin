...
a.innerHTML = 
	...
	'<button  class ="btn btn-prim  vi-VR-btnWdth-XL" id="gift_aid_submit"> Add to Gift Aid'+
	'</button>';

parent = document.getElementById("itemTitle");
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("lst icon");
		if (typeof elements[1] === 'undefined'){
			//continue;
		}
		else{
			elements = document.getElementsByClassName("lst icon")[1].getElementsByTagName('li');
		}
		//console.log("The small image information " + elements[0].innerHTML);
		for(i=0;i<elements.length;i++){
			...
			if (temp.endsWith('jpg')){
				temp =temp.replace("s-l64","s-l300");
		...
		if(imageurls.length == 0){
			elements = document.getElementById("icImg");
	...
	// get the product descriptions
		elements = document.getElementsByClassName("itemAttr")[0];
	...
	// get the product title
		...
		elements = document.getElementById("itemTitle");
	...
	// get the product price
		...
		elements = document.getElementById("prcIsum");
		if(!elements){
			elements = document.getElementById("mm-saleDscPrc"); 
		}
		price = elements.innerText;
		indexfirst=price.lastIndexOf("$");
		price=price.substring(indexfirst,price.length);
		if (price == ""){
			elements = document.getElementById("orgPrc");
			price = elements.innerText;
		}
		price=price.trim();
		price = price.replace('$','');
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementById("TopPanel");
         div.appendChild(e);
