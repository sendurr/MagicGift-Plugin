...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit" class="add_to_cart bottom_item" style="background-color:rgb(10, 146, 202); border-radius: 8px; font-size: 16px; color:white;">Add to Gift Aid </button>';

parent = document.getElementsByClassName("pinnedDiv")[1];
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("s7thumb");
		//console.log(elements[0].getAttribute("style"));
		for(i=0;i<elements.length;i++){
			src_elements = elements[i].getAttribute("style");
			indexfirst=src_elements.indexOf("url(\"http:");
			src_elements=src_elements.substring(indexfirst,src_elements.length);
			src_elements = src_elements.replace('url("http:','http:');
	...
	// get the product descriptions
		...
		description = description.replace('PRODUCT OVERVIEW','');
	...
	// get the product title
		...
		elements = document.getElementsByClassName("title")[0];
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementsByClassName("pdp_header")[0];
         div.appendChild(e);
