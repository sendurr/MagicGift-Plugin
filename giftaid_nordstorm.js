...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit" class="nui-button-v6 add-to-bag-button primary" style="background-color:rgb(200, 49, 30); border-radius: 8px; font-size: 16px; color:white;"> Add to Gift Aid'+
	'</button>';

parent = document.getElementsByClassName("product-title")[0];
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("content-wrapper")[0].getElementsByTagName('div');
		elements =elements[0].getElementsByTagName('li');
		//console.log(elements[0].innerHTML);
		for(i=0;i<elements.length;i++){
			src_elements = elements[i].innerHTML;
			indexfirst=src_elements.indexOf("src=\"");
			...
			src_elements=src_elements.substring(0,indexfirst);
			if (src_elements.endsWith('jpg')){
				temp = src_elements.replace('Mini','Large');
			}
			else{
				indexfirst=src_elements.indexOf("?");
	...
	// get the product descriptions
		elements = document.getElementsByClassName("product-details-and-care")[0];
	...
	// get the product title
		...
		elements = document.getElementsByClassName("product-title")[0];
	...
	// get the product price
		...
		elements = document.getElementsByClassName("price-current price-current-red")[0];
		if((typeof elements === 'undefined')){
			elements = document.getElementsByClassName("price-display-item regular-price")[0];
		}
		if(!elements){
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementsByClassName("nui-container-content")[0];
         div.appendChild(e);
