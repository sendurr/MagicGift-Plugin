...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit" class="btn-fox green addToCart gae-click*Product-Page*PrForm*Add-To-Cart"> Add to Gift Aid'+
	'</button>';

parent = document.getElementsByClassName("stageItem clearfix onStage")[0];
child_elements = parent.childNodes;
//console.log(child_elements[1]);
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("angles-vertical")[0].getElementsByTagName('li');
		//console.log(elements[0].innerHTML);
		for(i=0;i<elements.length;i++){
			src_elements = elements[i].innerHTML;
			indexfirst=src_elements.indexOf("background-image:url(");
			src_elements=src_elements.substring(indexfirst,src_elements.length);
			src_elements = src_elements.replace('background-image:url(',' ');
			indexfirst=src_elements.indexOf(")\"");
			temp=src_elements.substring(0,indexfirst);
			if (temp.endsWith('jpg')){
				temp = temp.replace('_THUMBNAILS','');
	...
	// get the product title
		...
		elements = document.getElementsByClassName("banner")[0];
	...
	// get the product price
		...
		elements = document.getElementById("priceSlot");
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementById("breadcrumbs");
         div.appendChild(e);
