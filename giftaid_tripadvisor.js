...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit"  class="commerce-action-button original large-action-button" style="background-color:rgb(10, 146, 202); border-radius: 8px; font-size: 16px; color:white;">Add to Gift Aid </button>';

parent = document.getElementById("headingWrapper");
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("vr-thumb-control");
		if (typeof elements[0] === 'undefined'){
			console.log('No additional images - assigned with mainimageurl');
		}
		else{
			elements  = document.getElementsByClassName("vr-thumb-control")[0].getElementsByTagName('img');
			
		}
		//console.log(elements[0].getAttribute("src"));
		for(i=0;i<elements.length;i++){
			src_elements = elements[i].getAttribute("src");
			temp = src_elements.replace('/vr-splice-m/','/vr-splice-j/');
			imageurls.push(temp.trim());
		}

		if(elements.length == 0){
			elements  = document.getElementById("curPhoto");
			src_elements = elements[i].getAttribute("src");
	...
	// get the product descriptions
		elements = document.getElementsByClassName("overviewCard")[0];
	...
	// get the product title
		...
		elements = document.getElementById("HEADING");
	...
	// get the product price
		...
		elements = document.getElementsByClassName("priceData")[0];
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementById("headingWrapper");
         div.appendChild(e);
