...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit" class="js-add-to-cart js-flyout-toggle js-btn-progress add-to-cart btn btn-primary btn-stacked btn-block" > Add to Gift Aid'+
	'</button>';

parent = document.getElementsByClassName("prod-title-section")[0];
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("slick-track")[0].getElementsByTagName('li');
		//console.log(elements[0].innerHTML);
		for(i=0;i<elements.length;i++){
			src_elements = elements[i].innerHTML;
			indexfirst=src_elements.indexOf("href");
			src_elements=src_elements.substring(indexfirst,src_elements.length);
			indexfirst=src_elements.indexOf(".jpg");
			src_elements=src_elements.substring(0,indexfirst+4);
			//console.log("The value of temp " + src_elements);
			indexfirst=src_elements.lastIndexOf("\"");
			temp=src_elements.substring(indexfirst,src_elements.length);
			temp = temp.replace('\"','');
	...
	// get the product descriptions
		elements = document.getElementsByClassName("ellipsis-content module")[0];
		description=elements.innerText; 
		indexfirst=description.lastIndexOf("manufacturer information.");
		description=description.substring(indexfirst,description.length);
		description = description.replace("manufacturer information.",'');
	...
	// get the product title
		...
		elements =  document.getElementsByClassName("prod-title-section")[0];
	...
	// get the product price
		...
		elements = document.getElementsByClassName("js-price-display Price Price--stylized  Price--large hide-content display-inline-m price-display")[0];
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementsByClassName("Grid prod-breadcrumb")[0];
         div.appendChild(e);
