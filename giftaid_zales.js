...
a.innerHTML = 
	...
	'<button  id="gift_aid_submit" width =100% style="background-color:red; border-radius: 8px; font-size: 16px; color:white;"> Add to Gift Aid'+
	'</button>';

parent = document.getElementById("BVCustomerRatings");
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementById("productAltImages").getElementsByTagName('li');
		for(i=0;i<elements.length;i++){
			...
			if (temp.endsWith('jpg')){
				temp =temp.replace("th.jpg","t400.jpg");
	...
	// get the product descriptions
		elements = document.getElementById("productLongDesc");
	...
	// get the product price
		...
		elements = document.getElementById("productPriceCell");
		price = elements.innerText;
		indexfirst=price.lastIndexOf("$");
		price=price.substring(indexfirst,price.length);
		if (price == ""){
			elements = document.getElementsByClassName("ourprice14")[0];
			price = elements.innerText;
		}
		price=price.trim();
		price = price.replace('$','');
