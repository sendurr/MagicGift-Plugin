...
a.innerHTML = 
	...
	'<button class="btn btn-primary btn-wide" id="gift_aid_submit" width =100% > Add to Gift Aid'+
	'</button>';
...
button.addEventListener ("click", function() {
 	...
 	// get the main product image & additional product images
		...
		elements  = document.getElementsByClassName("navThumbs");
		if (typeof elements[0] === 'undefined'){
			single_image=1;
		}
		else{
			elements  = document.getElementsByClassName("navThumbs")[0].getElementsByTagName('li');
		}
		//console.log("The image link " + elements[0].innerHTML);
		...
		for(i=0;i<elements.length;i++){
			...
			indexlast=src_elements.lastIndexOf("src");
			...
			if(temp.search("ProductImageCompressAll35") >=0){
				temp= temp.replace("ProductImageCompressAll35","ProductImage")
			}
			else{
				indexlast=temp.indexOf("?");
				//console.log("Image url before" + (i+1) + " " + temp );
				temp = temp.substring(0,indexlast);
				//console.log("Image url after" + (i+1) + " " + temp );
			}
			imageurls.push(temp)	
		}

		if(single_image ==1){
			elements  = document.getElementsByClassName("objImages")[0].getElementsByTagName('img');
			temp = elements[0].src;
			imageurls.push(temp)
		}
	...
	// get the product descriptions
		elements = document.getElementsByClassName("grpBullet")[0];
	...
	// get the product title
		...
		elements = document.getElementsByClassName("wrapper")[0].getElementsByTagName('span');
	...
	// get the product price
		...
		elements = document.getElementsByClassName("price-current")[0];
	...
	function showmessage() {
		...
		'<div class="alert alert-success alert-dismissable">'+
         ...
         '</div>';
         // e.appendTo("#navbar");
         div=document.getElementsByClassName("header-inner")[0];
         div.appendChild(e);
